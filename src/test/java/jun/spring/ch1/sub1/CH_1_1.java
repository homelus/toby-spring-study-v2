...
public class CH_1_1 {
    ...
    public void GENERIC_XML_APPLICATION_CONTEXT_테스트() {
        GenericApplicationContext ac = new GenericXmlApplicationContext("/etc/genericApplicationContext.xml");
        Hello hello = ac.getBean("hello", Hello.class);
        hello.print();
    ...
    public void APPLICATION_CONTEXT_계층구조_적용_테스트() {
        // 루트 컨텍스트 생성
        ApplicationContext parent = new GenericXmlApplicationContext("/etc/parentContext.xml");
        // 자식 컨텍스트 생성
        GenericApplicationContext child = new GenericApplicationContext(parent);
        XmlBeanDefinitionReader reader = new XmlBeanDefinitionReader(child);
        // 부모에게는 printer 가 있고 자식에게는 printer 가 없음, print 빈을 찾을 것인가?
        reader.loadBeanDefinitions("/etc/childContext.xml");
        child.refresh();
